...
# 1 Binary Classification - Demographics - XGBoost Model Predictions
...
result_D_xgb <- computeSumStats( yp_dat=merged_geno_df %>% pull(`.pred_No AD`), 
...
###################################################################################
# D_xgb_plot_v1 <- plot(-log10(result_true_labels$P), -log10(result$P))
# print(D_xgb_plot_v1)

# Create a data frame from your results
D_xgb_df <- data.frame(
  snp_effect_size = -log10(result_true_labels$P),
  D_xgb = -log10(result_D_xgb$P)
)

# Calculate the correlation
correlation_value <- cor(D_xgb_df$snp_effect_size, D_xgb_df$D_xgb)

# Print the correlation value
# print(paste("Correlation: ", round(correlation_value, 3)))

# Create the plot with an identity line and correlation annotation
D_xgb_plot_v2 <- ggplot(D_xgb_df, aes(x = snp_effect_size, y = D_xgb)) +
  ...
  labs(
    ...
    y = "Demographics Predictions XGB Model)"
  ) +
  theme_minimal() +
  theme(
  ...
  annotate("text", x = max(D_xgb_df$snp_effect_size) * 0.7, 
           y = max(D_xgb_df$D_xgb) * 0.9, 
...
# D_xgb_plot2_v1 <- plot(-log10(result_true_labels$P[-I]), -log10(result$P[-I]))
# print(D_xgb_plot2_v1)

# Create a data frame for plotting with filtered values
D_xgb_df2 <- data.frame(
  snp_effect_size_filtered = -log10(result_true_labels$P[-I]),
  D_xgb_filtered = -log10(result_D_xgb$P[-I])
)

# Calculate the correlation
correlation_value <- cor(D_xgb_df2$snp_effect_size_filtered, D_xgb_df2$D_xgb_filtered)


# Create the plot with appropriate labels and styles
D_xgb_plot2_v2 <- ggplot(D_xgb_df2, aes(x = snp_effect_size_filtered, y = D_xgb_filtered)) +
  geom_point(color = 'red', size = 2) + # Add points
  labs(
    title = "Filtered Regression Analysis Results",
    x = "SNP Effect Size (Filtered)",
    y = "D_xgb Predictions (Filtered)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5),
  ...
  annotate("text", x = max(D_xgb_df2$snp_effect_size_filtered) * 0.7, 
           y = max(D_xgb_df2$D_xgb_filtered) * 0.9, 
           label = paste("Correlation: ", round(correlation_value, 3)), 
           color = "black", size = 5)
# Display the plot
print(D_xgb_plot2_v2)
```
...
Fh_glm_plot_v2 <- ggplot(Fh_glm_df, aes(x = snp_effect_size, y = Fh_glm)) +
  ...
  labs(
    title = "Regression Analysis Results",
    x = "SNP Effect Size (Coefficient of Regression Actual Labels)",
    y = "Demographics Predictions GLM Model"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5),
...
# 3 Binary Classification - Demographics + Family History - XGBoost Model
...
result_DFh_xgb <- computeSumStats( yp_dat=merged_geno_df %>% pull(`.pred_No AD`), 
                           merged_geno_df %>% select(-`.pred_No AD`), 
                           is_binary=F)
...
print(result_DFh_xgb)


###################################################################################
DFh_xgb_df <- data.frame(
  snp_effect_size = -log10(result_true_labels$P),
  DFh_xgb = -log10(result_DFh_xgb$P)
)

# Calculate the correlation
correlation_value <- cor(DFh_xgb_df$snp_effect_size, DFh_xgb_df$DFh_xgb)

# Create the plot with an identity line and correlation annotation
DFh_xgb_plot_v2 <- ggplot(DFh_xgb_df, aes(x = snp_effect_size, y = DFh_xgb)) +
  geom_point(color = 'blue', size = 2) + # Add points
  labs(
    title = "Regression Analysis Results",
    x = "SNP Effect Size (Coefficient of Regression Actual Labels)",
  ...
  annotate("text", x = max(DFh_xgb_df$snp_effect_size) * 0.7, 
           y = max(DFh_xgb_df$DFh_xgb) * 0.9, 
           label = paste("Correlation: ", round(correlation_value, 3)), 
           color = "black", size = 5)

# Display the plot
print(DFh_xgb_plot_v2)


###################################################################################
I <- which(result_true_labels$P < 0.0000001)

DFh_xgb_df2 <- data.frame(
  snp_effect_size_filtered = -log10(result_true_labels$P[-I]),
  DFh_xgb_filtered = -log10(result_DFh_xgb$P[-I])
)

# Calculate the correlation
correlation_value <- cor(DFh_xgb_df2$snp_effect_size_filtered, DFh_xgb_df2$DFh_xgb_filtered)

DFh_xgb_plot2_v2 <- ggplot(DFh_xgb_df2, aes(x = snp_effect_size_filtered, y = DFh_xgb_filtered)) +
  geom_point(color = 'red', size = 2) + # Add points
  labs(
    title = "Filtered Regression Analysis Results",
    x = "SNP Effect Size (Filtered)",
    y = "DFh_xgb Predictions (Filtered)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5),
  ...
  annotate("text", x = max(DFh_xgb_df2$snp_effect_size_filtered) * 0.7, 
           y = max(DFh_xgb_df2$DFh_xgb_filtered) * 0.9, 
           label = paste("Correlation: ", round(correlation_value, 3)), 
           color = "black", size = 5)
# Display the plot
print(DFh_xgb_plot2_v2)
...
DGFh_glm_plot2_v2 <- ggplot(DGFh_glm_df2, aes(x = snp_effect_size_filtered, y = DGFh_glm_filtered)) +
  ...
  labs(
    title = "Filtered Regression Analysis Results",
    x = "SNP Effect Size (Filtered)",
    y = "DGFh_glm Predictions (Filtered)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5),
...
RA_BC_plot1 <- grid.arrange(D_xgb_plot_v2, Fh_glm_plot_v2, DFh_xgb_plot_v2, DGFh_glm_plot_v2, ncol = 2, nrow = 2)
...
RA_BC_plot2 <- grid.arrange(D_xgb_plot2_v2, Fh_glm_plot2_v2, DFh_xgb_plot2_v2, DGFh_glm_plot2_v2, ncol = 2, nrow = 2)
