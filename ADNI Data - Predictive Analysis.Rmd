...
# Install the parsnip package, for modeling. Useful because it provides a consistent interface to many different types of models.
...
# Install and load the yardstick package, which provides tools for evaluating models with tidy data principles.
install.packages("yardstick")
library(yardstick)


# Install the following packages: aorsf, censored, glmnet, modeldatatoo, and tidymodels.
...
library(modeldata) \# WHY DO I NEED THIS?
...
# View predictions
final_res %>% collect_predictions() %>% View()

# Plot the ROC curve for the final model's predictions
final_res %>%
  collect_predictions() %>%
  mutate(# Convert 'event' to numeric (0,1)
         pred_class_numeric = as.numeric(.pred_class)) %>%
...
# Trying all Recipes on Tuned Models
...
# Define another recipe for preprocessing the data using familial history predictors to predict 'event'
surv_recipe1 <- recipe(event ~ fam_hist_dad_dem + fam_hist_dad_ad + fam_hist_mum_dem + fam_hist_mum_ad, data = train_data)
...
# Define a recipe for preprocessing the data using gender, age and _____ and education level to predict 'event'
```
