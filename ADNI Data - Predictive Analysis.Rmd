...
# Surv_recipe - LOGISTIC REGRESSION Model Tuning
...
# Define a logistic regression model for classification
logistic <- glm(event ~ ., data=surv_df, family="binomial")
summary(logistic)
```
...
# Define a logistic regression model for classification with regularization
# alpha = 1 for Lasso (L1), alpha = 0 for Ridge (L2), alpha in between for Elastic Net
...
# Penalty parameter (lambda) and mixture parameter (alpha)
tune_grid <- grid_regular(penalty(range = c(-3, 0)), mixture(range = c(0, 1)), levels = 10)
...
glmnet_tune <- tune_grid(
  ...
  resamples = cv_folds,
  grid = tune_grid,
  control = control_grid(save_pred = TRUE)) # Ensure roc_auc and accuracy are functions from yardstick


# Collect metrics
glmnet_metrics <- collect_metrics(glmnet_tune)
glmnet_metrics
# Plot ROC AUC for the different penalty values
roc_plot <- glmnet_metrics %>%
  filter(.metric == "roc_auc") %>%
  ggplot(aes(penalty, mean, color = as.factor(mixture))) +
  geom_pointrange(aes(ymin = mean - std_err, ymax = mean + std_err)) +
  labs(title = "ROC AUC vs. Penalty", x = "Penalty (log scale)", y = "ROC AUC")

# Plot accuracy for the different penalty values
accuracy_plot <- glmnet_metrics %>%
  filter(.metric == "accuracy") %>%
  ggplot(aes(penalty, mean, color = as.factor(mixture))) +
  geom_pointrange(aes(ymin = mean - std_err, ymax = mean + std_err)) +
  labs(title = "Accuracy vs. Penalty", x = "Penalty (log scale)", y = "Accuracy")
...
final_glmnet_workflow <- finalize_workflow(glmnet_workflow, best_params)

# Fit the final model on the entire training set
final_glmnet_fit <- fit(final_glmnet_workflow, data = train_data)

# Predict on the test set and evaluate
test_predictions <- predict(final_glmnet_fit, test_data, type = "prob") %>%
  bind_cols(test_data)

roc_curve(test_predictions, event, .pred_AD) %>%
  autoplot()

# Calculate and print the ROC AUC on the test set
test_results <- roc_auc(test_predictions, event, .pred_AD)
print(test_results)
...
# SURVIVAL ANALYSIS MODEL - censored regression Weibull distribution
...
# Backtrace:
#  1. ... %>% ggplot(aes(.eval_time, mean))
#  5. tune:::collect_metrics.tune_results(survreg_res)
#  6. tune::estimate_tune_results(x)

#-------------------------#

#troubleshooting: 
show_notes(.Last.tune.result) #OR you can do: show_notes(survreg_res)
...
summary(surv_df$VISCODE)

   # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
   # 0.00   24.00   36.00   52.66   84.00  144.00 
...
# SURVIVAL MODEL - WITHOUT SURIVIVAL TIMES / VISCODE = 0 as this may be causing the issue with the weibull distribution
...
# Re-create the survival object and split the data again
surv_df1 <- surv_df %>% 
...
# Initial Low Scores: At the beginning of the evaluation period (near time 0), the Brier score is low, indicating high accuracy in the short-term predictions. This is expected because there are likely few events (deaths or disease progressions) early on, so the survival probabilities are close to 1, leading to low Brier scores.
# 
# Increasing Brier Scores: As time progresses, the Brier score increases. This suggests that the accuracy of the survival predictions decreases over time. This could be due to a number of factors such as more events occurring, making predictions harder, or the model not capturing long-term survival trends well.
# 
# Fluctuations: The score fluctuates over time, which indicates variability in the model's accuracy at different time points. This can occur if the number of events changes significantly at different times or if the model's assumptions do not hold uniformly across all time points.
# 
# High Brier Score Near End: Towards the end of the evaluation period, the Brier score spikes, indicating poor predictive accuracy. This could be due to a small number of subjects remaining in the study, leading to higher uncertainty and less reliable predictions.
```

# SURVIVAL ANALYSIS MODEL - censored regression COXPH distribution
...
# Specify the Cox proportional hazards model
coxph_spec <- proportional_hazards() %>%
  set_engine("survival") %>%
  set_mode("censored regression")

# Update the workflow to use the Cox model
coxph_wflow <- workflow() %>%
  add_recipe(surv_rec) %>%
  add_model(coxph_spec)

# Fit and evaluate the Cox model
coxph_res <- fit_resamples(
  coxph_wflow,
  resamples = surv_rset,
  metrics = survival_metrics,
  eval_time = evaluation_time_points, 
...
# Collect and inspect predictions
coxph_preds <- collect_predictions(coxph_res)
coxph_preds

# Plot the ROC AUC for the Cox model
collect_metrics(coxph_res) %>%
  filter(.metric == "roc_auc_survival") %>%
  ggplot(aes(.eval_time, mean)) +
  geom_line() +
  labs(x = "Evaluation Time", y = "Area Under the ROC Curve")

# Plot the Brier score for the Cox model
collect_metrics(coxph_res) %>%
  filter(.metric == "brier_survival") %>%
  ggplot(aes(.eval_time, mean)) +
  geom_line() +
