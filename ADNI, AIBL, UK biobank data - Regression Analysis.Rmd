...
gg_miss_var(UKBB) + ggtitle("Missing Values in Each Column of UKB Dataset")
vis_miss(UKBB) + labs(title = "Heatmap of Missing Values in UKB Dataset")
...
result <- computeSumStats( merged_geno_df %>% pull(-`.pred_No AD`), 
                           merged_geno_df %>% select(-`.pred_No AD`))
print(result)


sum(preds_ukbb$ID %in% geno_covar$FID)/length(preds_ukbb$ID)
[1] 0.8646288

sum(geno_covar$FID %in% preds_ukbb$ID)/length(geno_covar$FID)
[1] 0.1188595
...
snpfile <- readRDS("ukb_adni_genotypes_58_snps_v2.RDS")
# View(snpfile)


# First, rename the 'ID' column in the data dataset to 'FID' to match the snpfile dataset
data$FID <- data$ID

# Merge keeping only rows with matching FID/ID
geno_covar <- merge(snpfile, data, by = "FID")

# names(geno_covar)


# # Filter rows where FH.x and FH.y are not the same
# different_rows <- geno_covar[geno_covar$SEX.x != geno_covar$SEX.y, ]
# different_rows
# 
# # Filter rows where FH.x and FH.y are not the same
# different_rows <- geno_covar[geno_covar$conv.x != geno_covar$conv.y, ]
# different_rows
...
geno_covar <- geno_covar %>% select(-FH.x, -AGE.x, -Assess_center, - APOE4.x, APOE4_e4e4, -SEX.x, -FH_na.x, -ID, -FH_na.x)

geno_covar <- geno_covar %>% select(-conv.x)
...
# Define the dependent variable 'yp' as the conversion status 'conv.y'
yp <- geno_covar$conv.y
...
```{r}
# Assuming `result` contains the output of computeSumStats with SNP names, p-values
# We will calculate -log10 of the p-values and plot them against Family History

# Step 1: Calculate -log10(p-values)
result$log10_P <- -log10(result$P)

# Step 2: Visualize the log10(p-values)
library(ggplot2)

# Scatter plot of -log10(p-values) across SNPs
ggplot(result, aes(x = rs, y = log10_P)) +
  geom_point() +
  labs(title = "-log10(p-values) for SNPs",
       x = "SNPs",
       y = "-log10(p-values)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),  # Rotate SNP names if needed
...
# Step 3: Create a family history comparison - assuming FH.y is binary (0 or 1)
# Create an aggregated dataset based on family history (if available per SNP)

# Example of plotting log10(P) by Family History if aggregated at the individual level
# merged_result <- merge(result, geno_covar[, c("FID", "FH.y")], by.x = "rs", by.y = "FID")

# Example scatter plot of log10(p-values) vs family history, assuming a correct merge
ggplot(merged_result, aes(x = FH.y, y = log10_P)) +
  geom_point() +
  labs(title = "-log10(p-values) vs Family History",
       x = "Family History (FH.y)",
       y = "-log10(p-values)") +
  theme_minimal()
