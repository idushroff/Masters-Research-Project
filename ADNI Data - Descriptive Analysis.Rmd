...
# get the median: survival time t at which S(t) = 0.5 or the time at which 50% of those at risk are expected to still be alive. 
KM <- survfit(Surv(VISCODE, event) ~ 1, data=surv_df)
print(KM) 
# unable to get median. why?
...
adni.cox <- coxph(Surv(VISCODE, event) ~ PTGENDER + AGE + APOE4 + MMSE + mPACCtrailsB + fam_hist_dad_dem + fam_hist_dad_ad + fam_hist_mum_dem + fam_hist_mum_ad, data=surv_df)
summary(adni.cox)

# coef = log hazards ratio
# exp(coef) = similar interpretation to odds ratios
```

```{r}

# we can tidy() the coxph() results and store them in a tibble data.frame
adni.cox.tibble <- tidy(adni.cox, exponentiate = T, conf.int = T)
adni.cox.tibble # display table

# doing this (storing the cox ph model results as a data.frame makes it easy to use ggplot2 to create plots of the hazard ratios and confidence intervals.)

# CREATE A FOREST PLOT
ggplot(adni.cox.tibble,
       aes(y=term, x=estimate, xmin=conf.low, xmax=conf.high)) + 
  geom_pointrange() + #plots center point (x) an range (xmin, xmax)
...
\# since the hazard ratio for gender contains one it is not considered to be a satistically significant variable
