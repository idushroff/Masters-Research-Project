...
for (model_name in names(fit_models)) {
  ...
  # c_ukbb <- concordance_survival(UKBB_subset %>% add_column(preds=preds_ukbb$.pred_time), 
  #                                    truth = event_surv, 
  #                                    estimate= preds)
  ...
  survival_metrics <- metric_set(brier_survival_integrated, 
                                 roc_auc_survival, 
                                 concordance_survival)
  surv_metrics_ukbb <- survival_metrics(preds_ukbb_surv, 
                                     truth = surv_obj, 
                                      pred)
  ...
  preds_ukbb_surv = preds_ukbb_survival
  preds_ukbb_surv$.pred_time = preds_ukbb$.pred_time
  preds_ukbb_surv$surv_obj = AIBL_subset$event_surv
  preds_ukbb_surv = .censoring_weights_graf(fit, preds_ukbb_surv)
