...
cox_grid <- expand.grid(
  ...
  levels = 1  # Since we're specifying exact values, just set levels to 1
)
...
for (i in seq_along(recipes_list)) {
  ...
  oblique_wflow <- workflow() %>%
    add_recipe(current_recipe) %>%
    add_model(rand_forest(min_n = tune(), 
                          max_depth = tune(), 
                          min_samples_split = tune()) %>% 
                set_engine("aorsf") %>% 
                set_mode("censored regression"))
  ...
  results_list[[paste0("Feature_Set_", i, "_Weibull")]] <- survreg_res
  results_list[[paste0("Feature_Set_", i, "_Cox")]] <- coxnet_res
  results_list[[paste0("Feature_Set_", i, "_RandomForest")]] <- oblique_res
}
...
SurvivalAllModels_0 <- ggplot(all_metrics, aes(x = recipe, y = mean, fill = Model)) +
  geom_boxplot() +
...
SurvivalAllModels_1 <- grid.arrange(SurvivalAllModels_0, recipe_legend1, ncol = 1, heights = c(3, 1))
print(SurvivalAllModels_1)
...
# Combine the box plot and the legend
SurvivalAllModels_2 <- grid.arrange(SurvivalAllModels_0, recipe_legend2, ncol = 1, heights = c(3, 1))
print(SurvivalAllModels_2)

# Save the updated plot
ggsave("SurvivalAllModels_0.png", plot = SurvivalAllModels_0, width = 10, height = 10)
ggsave("SurvivalAllModels_1.png", plot = SurvivalAllModels_1, width = 10, height = 10)
ggsave("SurvivalAllModels_2.png", plot = SurvivalAllModels_2, width = 10, height = 10)
