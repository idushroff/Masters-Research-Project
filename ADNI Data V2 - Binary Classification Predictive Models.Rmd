...
for (i in seq_along(recipes_list)) {
  ...
  # Select best models from tuned results
  best_rf_tuned <- select_best(rf_tuned_res, "roc_auc")
  best_xgb_tuned <- select_best(xgb_tuned_res, "roc_auc")

  # Fit the best Random Forest model
  best_rf_model <- finalize_workflow(rf_tuned_wflow, best_rf_tuned) %>%
                   fit(data = surv_df)

  # Fit the best XGBoost model
  best_xgb_model <- finalize_workflow(xgb_tuned_wflow, best_xgb_tuned) %>%
                    fit(data = surv_df)
...
# Initialize an empty data frame to store all metrics
all_metrics <- data.frame()
results_list$Recipe_1_Logistic
results_list$Recipe_1_Logistic$.predictions

results_list$Recipe_1_Logistic$.predictions[[1]]
results_list$Recipe_1_Logistic$.predictions[[2]]
...
# Running for one recipe to see the boxplots better
...
for (i in seq_along(recipes_list)) {
  ...
  rf_tuned_wflow <- workflow() %>%
    add_recipe(current_recipe) %>%
    add_model(rand_forest(trees = 1000, mtry = tune(), min_n = tune()) %>% 
                set_engine("ranger") %>% 
                set_mode("classification"))
  ...
  xgb_tuned_wflow <- workflow() %>%
    add_recipe(current_recipe) %>%
    add_model(boost_tree(trees = 1000, tree_depth = tune(), loss_reduction = tune(), mtry = tune(), min_n = tune(), learn_rate = tune()) %>% 
                set_engine("xgboost") %>% 
                set_mode("classification"))
  ...
  # Define evaluation metrics: ROC AUC, Brier Score, and Accuracy
  classification_metrics <- metric_set(roc_auc, brier_class, accuracy)
...
metric_plot <- all_metrics %>%
  ggplot(aes(x = recipe, y = mean, fill = model)) +
  geom_boxplot() +
  facet_wrap(~.metric, scales = "free_y") +
  labs(title = "Model Comparison Across Recipes", x = "Recipe", y = "Metric Value") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Print the metric plot
print(metric_plot)

# Density plot for all metrics
density_plot <- all_metrics %>%
  ggplot(aes(x = mean, color = model, fill = model)) +
  geom_density(alpha = 0.1) +
  facet_wrap(~.metric, scales = "free") +
  labs(title = "Density of Metrics Across Models", x = "Metric Value")
