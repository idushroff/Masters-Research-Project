...
# p=0.1 which is not less than 0.05 so we can't reject the null hypothesis. We cannot say that the survival for males and females is different!
...
We know that survival for ADNI participants depends on features: age, apoe, family history and so on.

So we will break things down and start by doing a standard two way analysis of variance (ANOVA)since we have both a continuous (age) and categorical predictor (apoe).

```{r}

set.seed(123) #This line sets the seed to 123. If you run any random number generation functions after this line, you'll get the same sequence of random numbers each time you run the code, as long as the seed remains the same. 

# Check that the event column is a factor 
is.factor(surv_df$event)

# Convert the event column to a factor
surv_df$event <- factor(surv_df$event, levels = c(0, 1), labels = c("No AD", "AD"))

# WE know that ~27% of the participants in this data go onto getting a AD diagnosis

surv_df %>% group_by() %>% count(DX)
surv_df %>% group_by() %>% count(event)
...
data_split <- initial_split(surv_df, prop = 3/4)
...
### USING RECIPE AND ROLES - TIDYMODELS WEBSITE

### USE A TRAINED WORKFLOW TO PREDICT

Our goal was to predict whether a person gets a diagnosis of AD or not.

1.  Built the model (`lr_mod`),

2.  Created a preprocessing recipe (`surv_rec`),

3.  Bundled the model and recipe (`surv_wflow`), and
...
Logistic regression model using the parsnip package:
...
We will want to use the recipe across several steps as we train and test our model. (we will process the recipe using the training set, apply the recipe to the training set and apply it to the test set) so to simplify this we will use a model workflow:
...
Now we will use the trained workflow (surv_fit) to predict with the unseen test data, which we will do with a single call to predict(). The predict() method applies the recipe to the new data then passes them to the fitted model.
...
surv_aug %>% 
  roc_curve(truth = event, '.pred_No AD') %>% 
  autoplot()
