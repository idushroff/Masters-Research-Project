...
# Logistic Regression with alpha and l1_ratio tuning
final_glm_wflow <- workflow() %>%
  add_recipe(recipes_list[["Fh"]]) %>%
  add_model(logistic_reg() %>%
              set_engine("glm") %>%
              set_mode("classification"))
  
final_glm_fit <- final_glm_wflow %>%
  fit(data = surv_df)

# Fit XGBoost using the best feature sets (D and DFh) and the best hyperparameters
final_xgb_wflow_D <- workflow() %>%
...
final_xgb_fit_D <- final_xgb_wflow_D %>%
  fit(data = surv_df)

final_xgb_fit_DFh <- final_xgb_wflow_DFh %>%
  fit(data = surv_df)

# Predict on the UKBB dataset using Logistic Regression
glm_preds <- predict(final_glm_fit, new_data = UKBB_subset, type = "prob") %>%
  bind_cols(predict(final_glm_fit, new_data = UKBB_subset)) %>%
  bind_cols(UKBB_subset)

# Predict on the UKBB dataset using XGBoost (D feature set)
xgb_preds_D <- predict(final_xgb_fit_D, new_data = UKBB_subset, type = "prob") %>%
  bind_cols(predict(final_xgb_fit_D, new_data = UKBB_subset)) %>%
  bind_cols(UKBB_subset)

# Predict on the UKBB dataset using XGBoost (DFh feature set)
xgb_preds_DFh <- predict(final_xgb_fit_DFh, new_data = UKBB_subset, type = "prob") %>%
  bind_cols(predict(final_xgb_fit_DFh, new_data = UKBB_subset)) %>%
  bind_cols(UKBB_subset)

# Evaluate Logistic Regression performance
glm_metrics <- glm_preds %>%
  metrics(truth = conv, estimate = .pred_class, .pred_AD) %>%
  filter(.metric %in% c("accuracy", "roc_auc"))

# Evaluate XGBoost (D) performance
xgb_metrics_D <- xgb_preds_D %>%
  metrics(truth = conv, estimate = .pred_class, .pred_AD) %>%
  filter(.metric %in% c("accuracy", "roc_auc"))

# Evaluate XGBoost (DFh) performance
xgb_metrics_DFh <- xgb_preds_DFh %>%
  metrics(truth = conv, estimate = .pred_class, .pred_AD) %>%
  filter(.metric %in% c("accuracy", "roc_auc"))
