...
Binaryglm_All_ordered <- ggplot(all_metrics, aes(x = feature_set, y = .estimate, fill = Model)) +
  geom_boxplot() +
  facet_wrap(~ .metric, scales = "free_y") +
...
Binaryglm_AUC_Only <- ggplot(roc_auc_metrics, aes(x = feature_set, y = .estimate, fill = Model)) +
  geom_boxplot() +
  labs(title = "Binary Classification - AUC-ROC Comparison Across Feature Sets", 
       x = "Feature Set(s)", 
       y = "AUC-ROC") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  theme_light(base_size = 13) + 
...
# Initialize an empty data frame for storing AUC results
roc_auc_results <- data.frame(
  Model = character(),
  Train_ROC_AUC = numeric(),
  UKBB_ROC_AUC = numeric(),
...
# Loop through each model to fit and predict
for (model_name in names(fit_models)) {
  fit <- fit_models[[model_name]]
  
  # Get predictions on the ADNI dataset (training set)
  preds_train <- predict(fit, new_data = surv_df, type = "prob") %>%
    bind_cols(predict(fit, new_data = surv_df)) %>%
    bind_cols(surv_df)
  ...
  roc_auc_train <- preds_train %>%
    roc_auc(truth = event, .pred_No_AD) # 'event' is the target column in surv_df
  
  # Generate predictions on UKBB dataset
  preds_ukbb <- predict(fit, new_data = UKBB_subset, type = "prob") %>%
    bind_cols(predict(fit, new_data = UKBB_subset)) %>%
    bind_cols(UKBB_subset)
  
  # Calculate ROC AUC for UKBB set
  roc_auc_ukbb <- preds_ukbb %>%
    roc_auc(truth = conv, .pred_No_AD)
  
  # Add prediction columns to UKBB_subset
  preds_ukbb <- preds_ukbb %>%
    mutate(prediction_label = .pred_class,
           prediction_prob_no_AD = .pred_No_AD,
           prediction_prob_AD = .pred_AD) %>%
    select(-.pred_No_AD, -.pred_AD, -.pred_class) # Optional: Drop intermediate prediction columns
  
  # Save the modified UKBB dataset with predictions
  output_filename <- paste0("UKBB_for_RA_", model_name, ".csv")
  write.csv(preds_ukbb, output_filename, row.names = FALSE)
  
  # Append ROC AUC metrics to results data frame
  roc_auc_results <- roc_auc_results %>%
    add_row(
      Model = model_name,
      Train_ROC_AUC = round(roc_auc_train$.estimate, 3),
...
for (model_name in names(fit_models)) {
  ...
  # Store the ROC AUC metric for training set
  roc_auc_train <- preds_train %>%
    roc_auc(truth = event, '.pred_No AD')
